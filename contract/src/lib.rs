...
impl CoinflipContract {
    ...
    // ── Issue #495: Leaderboard — ranking by wins, streaks, and volume ──────

    /// Get leaderboard sorted by longest streak (descending), paginated.
    pub fn get_leaderboard_by_streak(
        env: Env,
        offset: u32,
        limit: u32,
    ) -> soroban_sdk::Vec<LeaderboardEntry> {
        let lb = Self::load_leaderboard(&env);
        let len = lb.entries.len();
        if len == 0 || offset >= len { return soroban_sdk::Vec::new(&env); }
        let mut idx: soroban_sdk::Vec<u32> = soroban_sdk::Vec::new(&env);
        for i in 0..len { idx.push_back(i); }
        // Insertion sort by longest_streak desc (max 100 entries)
        for i in 1..idx.len() {
            let mut j = i;
            while j > 0 {
                let a = lb.entries.get(idx.get(j - 1).unwrap()).unwrap();
                let b = lb.entries.get(idx.get(j).unwrap()).unwrap();
                if a.longest_streak < b.longest_streak {
                    let t = idx.get(j - 1).unwrap();
                    idx.set(j - 1, idx.get(j).unwrap());
                    idx.set(j, t);
                    j -= 1;
                } else { break; }
            }
        ...
        let page = limit.min(20);
        let end = (offset + page).min(idx.len());
        let mut result = soroban_sdk::Vec::new(&env);
        for i in offset..end { result.push_back(lb.entries.get(idx.get(i).unwrap()).unwrap()); }
        result
    }

    /// Get leaderboard sorted by total volume wagered (descending), paginated.
    pub fn get_leaderboard_by_volume(
        env: Env,
        offset: u32,
        limit: u32,
    ) -> soroban_sdk::Vec<LeaderboardEntry> {
        ...
        if len == 0 || offset >= len { return soroban_sdk::Vec::new(&env); }
        let mut idx: soroban_sdk::Vec<u32> = soroban_sdk::Vec::new(&env);
        let mut vols: soroban_sdk::Vec<i128> = soroban_sdk::Vec::new(&env);
        for i in 0..len {
            let e = lb.entries.get(i).unwrap();
            let ps = Self::load_player_stats(&env, &e.player);
            idx.push_back(i);
            vols.push_back(ps.total_wagered);
        }
        for i in 1..idx.len() {
            let mut j = i;
            while j > 0 {
                if vols.get(j - 1).unwrap() < vols.get(j).unwrap() {
                    let ti = idx.get(j - 1).unwrap();
                    let tv = vols.get(j - 1).unwrap();
                    idx.set(j - 1, idx.get(j).unwrap());
                    vols.set(j - 1, vols.get(j).unwrap());
                    idx.set(j, ti);
                    vols.set(j, tv);
                    j -= 1;
                } else { break; }
        ...
        let mut result = soroban_sdk::Vec::new(&env);
        for i in offset..end {
            let e = lb.entries.get(idx.get(i).unwrap()).unwrap();
            let ps = Self::load_player_stats(&env, &e.player);
            result.push_back(LeaderboardEntry {
                player: e.player,
                total_winnings: e.total_winnings,
                longest_streak: e.longest_streak,
                total_games: ps.games_played,
            });
        }
        result
    }

    /// Get leaderboard sorted by total wins (descending), paginated.
    pub fn get_leaderboard_by_wins(
        ...
        if len == 0 || offset >= len { return soroban_sdk::Vec::new(&env); }
        let mut idx: soroban_sdk::Vec<u32> = soroban_sdk::Vec::new(&env);
        let mut wins: soroban_sdk::Vec<u64> = soroban_sdk::Vec::new(&env);
        for i in 0..len {
            let e = lb.entries.get(i).unwrap();
            let ps = Self::load_player_stats(&env, &e.player);
            idx.push_back(i);
            wins.push_back(ps.wins);
        }
        for i in 1..idx.len() {
            let mut j = i;
            while j > 0 {
                if wins.get(j - 1).unwrap() < wins.get(j).unwrap() {
                    let ti = idx.get(j - 1).unwrap();
                    let tw = wins.get(j - 1).unwrap();
                    idx.set(j - 1, idx.get(j).unwrap());
                    wins.set(j - 1, wins.get(j).unwrap());
                    idx.set(j, ti);
                    wins.set(j, tw);
                    j -= 1;
                } else { break; }
    ...
    pub fn get_player_leaderboard_stats(env: Env, player: Address) -> PlayerLeaderboardStats {
        Self::load_player_leaderboard_stats(&env, &player)
    }

    /// Get total number of entries on the leaderboard.
    pub fn get_leaderboard_size(env: Env) -> u32 {
        Self::load_leaderboard(&env).entries.len()
    }
}
